...
/**
 * @brief Initialize the I2C master interface (idempotent).
 *
 * Safe to call more than once — touch_gt911_init() also calls this.
 */
DEV_I2C_Port DEV_I2C_Init()
{
    if (s_i2cReady && handle.bus) {
        return handle;
    }
    ...
    // Placeholder device — real addresses set via DEV_I2C_Set_Slave_Addr()
...
void DEV_I2C_Write_Byte(i2c_master_dev_handle_t dev_handle, uint8_t Cmd, uint8_t value)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "I2C write byte failed: %s", esp_err_to_name(err));
    }
}
...
uint8_t DEV_I2C_Read_Byte(i2c_master_dev_handle_t dev_handle)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "I2C read byte failed: %s", esp_err_to_name(err));
    }
    return data[0];
}
...
uint16_t DEV_I2C_Read_Word(i2c_master_dev_handle_t dev_handle, uint8_t Cmd)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "I2C read word failed: %s", esp_err_to_name(err));
        return 0;
    }
...
void DEV_I2C_Write_Nbyte(i2c_master_dev_handle_t dev_handle, uint8_t *pdata, uint8_t len)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "I2C write nbyte failed: %s", esp_err_to_name(err));
    }
}
...
void DEV_I2C_Read_Nbyte(i2c_master_dev_handle_t dev_handle, uint8_t Cmd, uint8_t *pdata, uint8_t len)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "I2C read nbyte failed: %s", esp_err_to_name(err));
    }
}
