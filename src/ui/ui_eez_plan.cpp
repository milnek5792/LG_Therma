...
constexpr int kTimeRowH = 48;
constexpr int kCasNameW = 32;
constexpr int kCasColX = kCasNameW + 4;
...
constexpr int kModalH = 320;
constexpr int kModalBtnH = 40;
constexpr int kModalBtnW = 64;
...
lv_obj_t* makeCasKarta(lv_obj_t* parent, int x, int y, int w, int h, uint8_t obdobi) {
  ...
  lv_obj_set_pos(lblOd, kCasColX, 4);
  lv_obj_set_width(lblOd, w - kCasColX - 2);
  ...
  lv_obj_set_pos(lblDel, kCasColX, 22);
  lv_obj_set_width(lblDel, w - kCasColX - 2);
...
void uiPlanCreate(void) {
  ...
  lv_obj_set_pos(planObj.lbl_col_den, kPad, 16);
  ...
  for (uint8_t d = 0; d < PLAN_POCET_DNU; ++d) {
    ...
    lv_obj_set_pos(planObj.lbl_dny[d], kPad, rowY + 16);
